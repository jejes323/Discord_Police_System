...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('사건삭제')
        .setDescription('[관리자] 특정 사용자의 사건을 삭제합니다.')
        .addUserOption(option =>
            option
                .setName('사용자')
                .setDescription('사건을 삭제할 사용자를 선택하세요')
                .setRequired(true)
        )
    ...
    async execute(interaction) {
        ...
        const selectMenu = new StringSelectMenuBuilder()
            .setCustomId('delete_case_select')
            .setPlaceholder('삭제할 사건을 선택하세요')
            .addOptions(options);

        const row = new ActionRowBuilder().addComponents(selectMenu);

        await interaction.reply({
            content: `🗑️ ${targetUser.tag}님의 사건 중 삭제할 사건을 선택해주세요:`,
            components: [row],
            ephemeral: true
        });
    }
